...
SKIP_DJANGO_MIGRATIONS=${SKIP_DJANGO_MIGRATIONS:-false}
if [[ $SKIP_DJANGO_MIGRATIONS = false ]]; then
	python /app/manage.py migrate
fi

# Load Fixtures
SKIP_DJANGO_FIXTURES=${SKIP_DJANGO_FIXTURES:-false}
if [[ $SKIP_DJANGO_FIXTURES = false ]]; then
	while read fixture; do
		python /app/manage.py loaddata $fixture
	done < <(find "/app/data" -type f -path "/app/data/*.json" | sort)
fi
